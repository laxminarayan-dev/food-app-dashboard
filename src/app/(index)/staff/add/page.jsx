...
const inputCls =
  "w-full px-3 py-2 border border-gray-200 rounded-lg text-sm text-slate-800 bg-white outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 placeholder:text-slate-300 transition";

// ── Moved OUTSIDE the main component to prevent remounting on every keystroke ──

const Section = ({ title, children }) => (
  <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
    <h2 className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-5">
      {title}
    </h2>
    {children}
  </div>
...
const AddEmployee = ({ onBack, onSuccess }) => {
  ...
  const validate = () => {
    ...
    if (!/^\d{12}$/.test(form.aadharNo.replace(/\s/g, "")))
      e.aadharNo = "Invalid Aadhar (12 digits).";
    if (!/^[A-Z]{5}[0-9]{4}[A-Z]$/.test(form.panNo.toUpperCase()))
      ...
      <div className="min-h-screen bg-transparent flex items-center justify-center">
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-10 flex flex-col items-center gap-4 max-w-sm w-full mx-4">
          ...
          <button
            ...
            className="mt-2 bg-indigo-600 text-white text-sm font-semibold px-6 py-2.5 rounded-xl hover:bg-indigo-700 transition-colors"
          >
            Back to Staff
          </button>
        </div>
    ...
    <div className="min-h-screen bg-transparent py-8 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-center gap-3 mb-7">
          <button
            onClick={() => router.push("/staff")}
            className="p-2 rounded-xl border border-gray-200 bg-white hover:bg-gray-50 text-slate-500 shadow-sm transition-colors"
          >
            <ArrowLeft size={16} />
          </button>
          <div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                ...
                return (
                  ...
                  >
                    {sel && (
                      <CheckCircle2
                        size={14}
                        className={`absolute top-2 right-2 ${r.ic}`}
                      />
                    )}
                    <div
                      className={`p-2.5 rounded-xl ${sel ? r.ibg : "bg-gray-100"}`}
                    >
                      <span className={sel ? r.ic : "text-slate-400"}>
                        {r.icon}
                      </span>
                    </div>
                    <div className="text-center">
                      <p
                        className={`text-sm font-semibold ${sel ? r.tc : "text-slate-700"}`}
                      >
                        {r.label}
                      </p>
          ...
          <Section title="Address">
            <div className="flex flex-col gap-4">
              ...
              >
                <input
                  value={capitalizeWords(form.address)}
                  onChange={(e) =>
                    set("address", capitalizeWords(e.target.value))
                  }
              ...
              <div className="grid grid-cols-3 gap-3">
                <Field label="City" error={errors.city}>
                  <input
                    value={capitalizeWords(form.city)}
                    onChange={(e) =>
                      set("city", capitalizeWords(e.target.value))
                    }
                    placeholder="City"
                    className={inputCls}
                  />
                ...
                <Field label="State" error={errors.state}>
                  <input
                    value={capitalizeWords(form.state)}
                    onChange={(e) =>
                      set("state", capitalizeWords(e.target.value))
                    }
                    placeholder="State"
                    className={inputCls}
                  />
          ...
          <div className="flex justify-end gap-3 pb-10">
            <button
              type="button"
              onClick={() => router.push("/staff")}
              className="px-5 py-2.5 text-sm font-semibold text-slate-600 border border-gray-200 rounded-xl bg-white hover:bg-gray-50 transition-colors"
            >
              Cancel
            </button>
            <button
